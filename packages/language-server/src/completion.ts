...
function generateCandidatesFromTokens(
  ...
  for (let [tokenType, followingTokens] of tokens) {
    if (allowedTokens.indexOf(tokenType) >= 0) {
      ...
      for (let followingTokenIndex of followingTokens) {
        const extraCandidate = (candidate +
          ' ' +
          lexer.vocabulary
            .getLiteralName(followingTokenIndex)
            ?.toUpperCase()
            .replace(/^'|'$/g, '')) as string;

        items.push(newKeywordItem(extraCandidate));
      }
    }
  }
  return items;
